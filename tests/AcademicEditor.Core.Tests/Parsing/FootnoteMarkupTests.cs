...
public sealed class FootnoteMarkupTests
{
    [Theory]
    [InlineData("[^1]: nota", "1")]
    [InlineData("[^nota]: texto", "nota")]
    [InlineData("[^abc]:    com folga", "abc")]
    ...
    /// <c>[^1]:algo</c> — que o autor pode ter escrito de propósito — deixaria de ser texto.
    /// </remarks>
    [Theory]
    [InlineData("[^1]:sem espaco")]
    [InlineData("[^]: identificador vazio")]
    [InlineData("[^1 2]: com branco no rotulo")]
    [InlineData("[^1] sem dois pontos")]
    [InlineData("nao comeca com [^1]: aqui")]
    ...
    /// <remarks>
    /// <b>Os colchetes são marcação e o identificador não é.</b> Escondida a marcação, a linha
    /// mostra o rótulo sobrescrito seguido do texto — que é como a nota se lê no pé da folha. Um
    /// rótulo escondido junto com a pontuação deixaria a nota sem dizer a que chamada responde.
    /// </remarks>
    [Fact]
    public void O_rotulo_aparece_sobrescrito_e_a_pontuacao_some()
    {
        var block = Assert.Single(MarkupParser.Parse("[^1]: nota").Blocks);
    ...
    [Theory]
    [InlineData("[^1]: nota")]
    [InlineData("[^abc]:   texto com **negrito** e $x$")]
    [InlineData(":-: [^1]: centralizada")]
    ...
    {
        var block = Assert.Single(MarkupParser.Parse(":-: [^1]: nota", TypographyPreset.Default).Blocks);
